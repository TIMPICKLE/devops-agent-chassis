...
1. **复用底盘装配真实业务**：AzureCodeAgent 已实现，复用业务服务与底盘接口，维护者确认实测可用。
2. **从结构化需求生成员工项目**：参考工厂读取 brief、源码输入约定、知识及完成标准，由模型生成 `employee.py`、`assembly.json` 和说明。当前限定 C++ include 修复、线性 3–8 节点。
3. **让同一个员工重复工作并维护升级**：同一生成物处理 pricing、scheduler 两份源码；知识更新保留人工代码与自定义文件，并检查新行为和旧任务。

生成与运行分开，是区别于一次性代码生成的重要展示点：生成一次 `employee.py`，随后用它执行不同输入；知识变化时不必重新生成全部代码。

### 已有真实 Actions 证据

| 链路 | 实际证据 | 结论范围 |
|---|---|---|
| 生成一次，运行两个源码项目 | [运行 34020348896](https://github.com/TIMPICKLE/devops-agent-chassis/actions/runs/34020348896) | 生成、两次运行和独立验收作业成功；是公开 C++ 参考案例 |
| 规范更新并保留人工修改 | [运行 34033606604](https://github.com/TIMPICKLE/devops-agent-chassis/actions/runs/34033606604) | 更新环节 0 次模型调用，同输入新旧行为对照及两个旧任务通过 |
| 当前底盘验收 | [运行 34245972616](https://github.com/TIMPICKLE/devops-agent-chassis/actions/runs/34245972616) | 对应底盘 `68264ed` 的分层回归及兼容性；不扩大为企业 live 证明 |

这些运行的源码快照不同。报告中的 historical live 证据只证明各自所测版本与任务，不能用普通回归绿色替代 live 运行。
...
开场用一条 Azure 工作项到 PR 的实际链路建立直观感受，再用 SonarQube 展示业务规模。随后揭示共同流程，引出底盘，并演示同一个员工处理新输入、按新规范改变行为。
