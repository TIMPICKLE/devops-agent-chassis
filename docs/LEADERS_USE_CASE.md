...
后续 T1–T5 维护还让技术人员更早发现配置冲突，并区分工具预检与模型服务请求失败。
启用独立工具并行后，报告记录实际批次和在途峰值，不能仅凭“配置了 4 个并发”就认定任务被加速。
需要更严格的来源与检查回执核验时，可另行接入[生产格式证据检查](PRODUCTION_EVIDENCE.md)；现有编译验收继续保留。
...
T1–T5 的后续维护通过合同与离线回归验证，记录见[维护索引](../changelog/docs-sync-t01-t05.md)。它们没有新增企业实机或付费模型验证，不替代上表两次历史实测。
