...
新增复用入口：[岗位配方库](ROLE_RECIPES.md)。配方声明任务源、编排、工具、验收、知识、失败及可观测组成，固定现有业务源码版本，按项目配置实例化；核心五系统不变。实例完整性、环境检查与真实业务验收分别记录。
...
服务器排错还需区分能力边界与代码缺陷：当前直接 API 适配器默认非流式，需要 SSE 的服务可显式开启[流式接收](STREAMING.md)；没有安全工具的装配提示不承诺自动降级；超时也不能直接归因于推理 token。核对方法及“有 diff 不等于业务修复成功”的验收边界见[模型接入与实测排错](MODEL_COMPATIBILITY.md)。
...
若模型服务要求流式，给下方每条生成/运行模型命令追加 `--stream`；OpenAI 网关明确不接受用量请求选项时再加 `--openai-omit-stream-usage`。核验与知识更新命令无需这些参数。流式接收和工具并行互相独立，详见[流式接入](STREAMING.md)。
